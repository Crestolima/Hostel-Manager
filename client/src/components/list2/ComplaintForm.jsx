...
import { Box, Paper, Typography, TextField, Button, CircularProgress, List, ListItem, ListItemText } from '@mui/material';
  ...
  useEffect(() => {
    if (regNo) {
      ...
      const fetchUserComplaints = async () => {
        try {
          const token = localStorage.getItem('token');
          const response = await axios.get(`http://localhost:5000/api/complaints/user/${regNo}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
    ...
    <Box sx={{ flexGrow: 1, p: 3 }}>
      ...
      <ComplaintFormCard>
        ...
        <TextField
          ...
          sx={{ mt: 2, mb: 2 }}
        />
        <Button variant="contained" color="primary" onClick={handleComplaintSubmit}>
          Submit Complaint
        </Button>
      </ComplaintFormCard>
